...
export async function POST(
  ...
  try {
    ...
    const result = await discoverCompany(companyName, id, Boolean(assessment.useMockData));
    ...
    addAuditEntry({
      ...
      reason: assessment.useMockData ? 'Company discovery completed via Mock Data' : 'Company discovery completed via LLM',
      actor: 'system',
    ...
    return NextResponse.json({ error: err.message || 'Company discovery failed' }, { status: 500 });
  }
}
